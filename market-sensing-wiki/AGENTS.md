...
- Signal 상세는 제목·분류 배지 바로 다음부터 구조화/산문 탭을 시작하세요. 판단 요약과
  ...
  둡니다.
- 산문 소제목은 금칙어 치환, Signal ID·주제어 하드코딩, 고정 fallback으로 만들지 마세요.
  LLM 편집 프롬프트에서 Signal별 독자 질문·잠정 결론·핵심 수치·판단 간극·바꿀 결정을
  먼저 정하고 각 장의 실제 결론에서 제목을 생성하게 하세요. 생성 뒤 다른 보고서에도
  그대로 붙일 수 있는 제목인지 본문과 대조해 스스로 다시 편집하게 하며, 공용 코드는
  형식·계층·계보 검증만 담당합니다.
