...
1. 최상위 `WIKI-SETTINGS.md`와 최근 성공 run을 읽고 `audit`의
...
6. 쿼리와 결과를 run JSON에 기록한다. 저장 작업이면 `results.new_claims`,
   `results.new_signals`, `signal_ids`를 함께 기록한다.
7. 후보를 ingest로 넘긴다.
...
2. 분석에는 확인된 변화·전달 메커니즘·조건부 시나리오·관찰 지표·다음 산출물·판단
   한계를 포함한다.
3. `add-signal --analysis-file <파일>`로 Signal과 Insight를 생성한다.
