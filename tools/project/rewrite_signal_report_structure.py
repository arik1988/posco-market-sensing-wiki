...
3. '확인된 변화 요약', '사업 판단 요약', '확인된 변화와 시점', '사업 영향 경로',
   ...
   제목을 쓰기 전에 각 보고서마다 독자 질문 1개, 잠정 결론 1개, 핵심 수치 최대 3개,
   기존 해석과 확인 사실의 간극, 바꿀 결정, 결론을 뒤집을 조건을 내부 논지 카드로
   정리한다. 이 카드를 출력하지 말고 각 장의 실제 결론에서 H2~H4를 생성한다.
...
15. 초안을 만든 뒤 소제목만 따로 읽고 자가 편집한다. 다른 입력 보고서에도 그대로 붙일
   수 있는 제목, 분석 절차나 저장 필드만 말하는 제목, 본문 첫 문장과 연결되지 않는 제목은
   해당 보고서의 시장 변화·영향 경로·결정이 드러나도록 다시 쓴다. 이 판단을 금칙어 치환,
   Signal ID·주제어 분기, 고정 fallback으로 대신하지 않는다.
16. 출력은 analysis_markdown과 짧은 edit_note만 반환한다. edit_note에는 논지 카드와
   소제목 자가점검을 거쳤다는 사실만 한 문장으로 적고 내부 추론은 노출하지 않는다.
"""
